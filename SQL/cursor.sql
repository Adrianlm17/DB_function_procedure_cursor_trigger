...
BEGIN
    ...
    OPEN autor_cursor;
    FETCH NEXT FROM autor_cursor INTO autor_id, autor_name;
    WHILE NOT done
    DO
        SELECT COUNT(*) INTO total_libros
        FROM libros
        WHERE autor_id = autor_id AND autor_id = libros.autor_id;
        SELECT CONCAT('El autor ', autor_name, ' ha escrito ', total_libros, ' libro/s.') as mensaje;
        FETCH NEXT FROM autor_cursor INTO autor_id, autor_name;
    END WHILE;
    CLOSE autor_cursor;
END$$
